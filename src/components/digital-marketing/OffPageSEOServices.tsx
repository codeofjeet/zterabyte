import {
  FiBarChart2,
  FiCheckCircle,
  FiGlobe,
  FiLink,
  FiMessageCircle,
  FiSearch,
  FiUsers,
} from "react-icons/fi";

const services = [
  {
    icon: FiLink,
    title: "Quality Link Building",
    description:
      "Build relevant and meaningful links from suitable websites to strengthen your website's authority and discoverability.",
    points: [
      "Relevant backlinks",
      "Quality link opportunities",
      "Natural link profile",
    ],
  },
  {
    icon: FiGlobe,
    title: "Website Outreach",
    description:
      "Identify relevant websites and digital platforms where your business can build useful connections and visibility.",
    points: [
      "Website research",
      "Relevant outreach",
      "Relationship building",
    ],
  },
  {
    icon: FiUsers,
    title: "Competitor Analysis",
    description:
      "Study competitor link profiles and online presence to identify useful opportunities for your own SEO strategy.",
    points: [
      "Backlink analysis",
      "Competitor research",
      "Opportunity discovery",
    ],
  },
  {
    icon: FiMessageCircle,
    title: "Brand Mentions",
    description:
      "Develop opportunities for your brand to be mentioned across relevant websites and online communities.",
    points: [
      "Brand visibility",
      "Online mentions",
      "Relevant platforms",
    ],
  },
  {
    icon: FiSearch,
    title: "Local SEO Signals",
    description:
      "Strengthen your local online presence with consistent business information and relevant local visibility activities.",
    points: [
      "Local presence",
      "Business listings",
      "Location relevance",
    ],
  },
  {
    icon: FiBarChart2,
    title: "Authority Monitoring",
    description:
      "Monitor important off-page SEO signals and review your link profile to identify improvements and potential issues.",
    points: [
      "Link monitoring",
      "Authority tracking",
      "Performance insights",
    ],
  },
];

export default function OffPageSEOServices() {
  return (
    <section className="overflow-hidden bg-[#f6fafd] py-16 sm:py-20 lg:py-24">
      <div className="mx-auto max-w-7xl px-5 lg:px-6">

        {/* Heading */}
        <div className="mx-auto max-w-3xl text-center">

          <span className="inline-flex items-center gap-2 rounded-full bg-[#eaf6ff] px-4 py-2 text-sm font-bold text-[#006cb5]">
            <FiBarChart2 />
            Off Page SEO Services
          </span>

          <h2 className="mt-5 text-3xl font-black leading-tight tracking-tight text-[#071827] sm:text-4xl lg:text-5xl">
            Build Authority
            <span className="block text-[#006cb5]">
              Beyond Your Website
            </span>
          </h2>

          <p className="mt-5 text-base leading-7 text-slate-600 sm:text-lg">
            Our off-page SEO activities focus on building relevant online
            connections, improving authority signals and strengthening your
            website's overall digital presence.
          </p>

        </div>

        {/* Services Grid */}
        <div className="mt-14 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">

          {services.map((service) => {
            const Icon = service.icon;

            return (
              <div
                key={service.title}
                className="group rounded-2xl border border-slate-200 bg-white p-6 shadow-sm transition duration-300 hover:-translate-y-1 hover:border-[#006cb5]/30 hover:shadow-lg sm:p-7"
              >

                {/* Icon */}
                <div className="flex h-12 w-12 items-center justify-center rounded-xl bg-[#eaf6ff] text-[#006cb5] transition duration-300 group-hover:bg-[#006cb5] group-hover:text-white">
                  <Icon className="text-xl" />
                </div>

                {/* Title */}
                <h3 className="mt-5 text-lg font-bold text-[#071827]">
                  {service.title}
                </h3>

                {/* Description */}
                <p className="mt-3 text-sm leading-6 text-slate-500">
                  {service.description}
                </p>

                {/* Points */}
                <div className="mt-5 space-y-2 border-t border-slate-100 pt-5">

                  {service.points.map((point) => (
                    <div
                      key={point}
                      className="flex items-center gap-2 text-xs font-medium text-slate-600"
                    >
                      <FiCheckCircle className="shrink-0 text-[#006cb5]" />
                      {point}
                    </div>
                  ))}

                </div>

              </div>
            );
          })}

        </div>

        {/* Bottom Highlight */}
        <div className="mt-10 rounded-2xl border border-[#dbeeff] bg-white p-6 shadow-sm sm:p-7">

          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">

            <div className="flex items-start gap-4">

              <div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl bg-[#eaf6ff] text-[#006cb5]">
                <FiLink />
              </div>

              <div>
                <h3 className="text-lg font-bold text-[#071827]">
                  Quality Over Quantity
                </h3>

                <p className="mt-1 max-w-2xl text-sm leading-6 text-slate-500">
                  Effective off-page SEO focuses on relevant, trustworthy
                  connections rather than simply collecting a large number of
                  links.
                </p>
              </div>

            </div>

            <div className="flex shrink-0 items-center gap-2 text-sm font-bold text-[#006cb5]">
              <FiCheckCircle />
              Authority Focused
            </div>

          </div>

        </div>

      </div>
    </section>
  );
}